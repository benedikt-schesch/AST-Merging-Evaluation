...
# Print file names followed by file content, in case a diff fails.
	...
	(cd results-small && cat result.csv | rev | cut -d, -f13-200 | rev > result-without-times.csv)
	(cd results-small && cat filtered_result.csv | rev | cut -d, -f13-200 | rev > filtered_result-without-times.csv)
