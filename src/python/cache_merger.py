...
if __name__ == "__main__":
    ...
    test_results_path = os.path.join(args.cache_path, "test_result")
    for file in tqdm(os.listdir(test_results_path)):
        if file.endswith(".txt") and not file.endswith("explanation.txt"):
            ...
            if not os.path.exists(target_file):
                ...
                test_state1, _ = read_cache(str(target_file))
                test_state2, _ = read_cache(str(source_file))
    ...
    merge_diff_results = os.path.join(args.cache_path, "merge_diff_results")
    ...
    # Merge test results
    merge_test_results_path = os.path.join(args.cache_path, "merge_test_results")
