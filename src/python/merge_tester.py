...
if __name__ == '__main__':
    ...
    for idx,row in df.iterrows():
        ...
        for idx2, row2 in merges.iterrows():
            if (not pass_test(repo_name,row2["left"])) or (not pass_test(repo_name,row2["right"])):
                continue
            #Git Merge
            ...
            if intelli_merge == 0:
                intelli_merge = subprocess.run([pwd+"/src/scripts/tester.sh","merge_repo/base"]).returncode+1
